...
void split(std::vector<float> &cdwd,
  ...
  *****/
  ...
  for (i = 0; i < dim; i++) {
    cdwd[newcdwd_baseidx + i] = cdwd[cdwd_baseidx + i] +
        stddev[stddev_baseidx + i] * mult_offset * (0.25 + drand48() / 2.0);
  }
}
               ...
               int dim)
{
  ...
  for (int i = 0; i < dim; i++)
    mse += (vc[vc_baseidx + i] - cdwd[cdwd_baseidx + i]) *
        (vc[vc_baseidx + i] -cdwd[cdwd_baseidx+i]);
