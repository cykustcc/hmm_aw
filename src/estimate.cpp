...
// *loglikehd is the log likelihood for the entire sequence
{
  ...
  for (int l=0; l<numst; l++, a++) {
    if (*a>0.0)
      thetalog[l]=log(*a)+gauss_pdf_log(u, md.stpdf[l], 0);
    else
      thetalog[l]=-HUGE;
  }
...
//  v3=dbpt[0];
  ...
  for (int m=0; m<numst; m++) {
    if (dbpt[baseidx+m]>v3) v3=dbpt[baseidx+m];
  }
  for (int m=0,v1=0.0; m<numst; m++) {
    v1+=exp(dbpt[baseidx+m]-v3);
  }
  v3+=log(v1);
             ...
             std::vector<int> &nstpercls)
{
  ...
  for (int i=0;i<numcls;i++) {
    for (int j=0,k=0,v1=0.0;j<numst;j++) {
      if (md.stcls[j]==i) {
        prior[i][k]=md.a00[j];
        v1+=prior[i][k];
      ...
      for (k1=0; k1<dim; k1++)
        for (k2=0; k2<dim; k2++) {
          if (!ranflag) //dilate slightly
            md.stpdf[m].sigma[k1][k2]=sigma[k1][k2]*lambda+(1.1-lambda)*sigcom[k1][k2];
          else
            md.stpdf[m].sigma[k1][k2]=sigma[k1][k2]*lambda+(1.0-lambda)*sigcom[k1][k2];
               ...
               std::vector<double> &lsum)
{
  ...
  for (int n=0;n<numcls;n++) {
    for (int l=0, v1=0.0, k=0; l<numst;l++) {
      if (stcls[l]!=n)
        bnl[n][l]=0.0;
      else {
        bnl[n][l]=lsum[l];
...
// Input wt[nseq] gives a weight to each sequence, normally it contains all 1
{
  ...
  while (ite<minite || twomdflag==0 || ratio>epsilon) {
    ...
    fprintf(stderr, "After asum adjustment\n");

    for (int t=0,lhsum=0.0;t<nseq;t++) lhsum+=wt[t]*loglikehd[t];
...
HmmModel& hmmfit(std::vector<std::vector<float>> &u,
                 int nseq,
                 std::vector<int> &len,
                 int dim,
                 int numst,
