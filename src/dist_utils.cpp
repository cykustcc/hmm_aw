  ...
  for (size_t i = 0; i < m; ++i)
    for (size_t j = 0; j < n; ++j)
      for (size_t k = 0; k < d; ++k) {
        C[j * m + i] += (A.stpdf[i].mean[k] - B.stpdf[j].mean[k]) *
            (A.stpdf[i].mean[k] - B.stpdf[j].mean[k]);
        C[j * m + i] += A.stpdf[i].sigma[k][k] + B.stpdf[j].sigma[k][k] -
            2 * sqrt(A.stpdf[i].sigma[k][k] * B.stpdf[j].sigma[k][k]);
      }
}
